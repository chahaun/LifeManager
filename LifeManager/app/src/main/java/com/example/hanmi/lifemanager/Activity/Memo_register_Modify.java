...
public class Memo_register_Modify extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.schedule_register_modify);
        ...
        if(intent != null) {
            MemoID = intent.getLongExtra("cId", -1);
            title.setText(intent.getStringExtra("cTitle"));
            contents.setText(intent.getStringExtra("cContents"));
            String formatDate = Integer.toString(intent.getIntExtra("cYear", 2018))+"년"
                    +Integer.toString(intent.getIntExtra("cMonth", 1))+"월"
                    +Integer.toString(intent.getIntExtra("cDay",1))+"일"
                    +Integer.toString(intent.getIntExtra("cHour",1))+"시"
                    +Integer.toString(intent.getIntExtra("cMinute",1))+"분";
            date_button.setText(formatDate);
            saveYear = intent.getIntExtra("cYear", 2018);
            saveMonth = intent.getIntExtra("cMonth", 1);
            saveDay = intent.getIntExtra("cDay", 1);
            saveHour = intent.getIntExtra("cHour", 1);
            saveMinute = intent.getIntExtra("cMinute", 1);

            if(intent.getStringExtra("cWL").equals("Work"))  work_and_life.check(R.id.radio_work);
            else if (intent.getStringExtra("cWL").equals("Life"))  work_and_life.check(R.id.radio_life);
            switch (intent.getStringExtra("cScope")) {
                case "전체공개":
                    scope.check(R.id.radio_all);
                    break;
                case "친구에게만 공개":
                    scope.check(R.id.radio_friend);
                    break;
                case "나만보기":
                    scope.check(R.id.radio_noOpen);
                    break;
            }
        }
        ...
        result_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                memo_intent.putExtra("id", MemoID);
        ...
        date_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                hide_Key_Board();
                Intent intent = new Intent(Memo_register_Modify.this, Time_Calendar.class);
